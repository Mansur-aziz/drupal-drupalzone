...
class TutorialBatchGenerator {
  ...
  Write a detailed (at least 2000 words) lesson for: "$topic_title".
  ...
  PROMPT;
    ...
    // }
    if ($run_status !== 'completed') {
      $error_reason = $status_data['last_error']['message'] ?? 'Unknown error';
      \Drupal::logger('tutorial_article_generator')->error('Run failed with status: @status. Reason: @reason', [
        '@status' => $run_status,
        '@reason' => $error_reason,
      ]);
      return;
